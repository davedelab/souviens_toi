...
def clip_to_markdown(clip: Dict[str, Any]) -> str:
    ...
    date = dt.datetime.fromtimestamp(clip.get("ts", 0), tz=dt.timezone.utc).strftime(
        "%Y-%m-%d %H:%M"
    )
    tags = [
        t.strip()
        for t in clip.get("tags", "").replace(";", ",").split(",")
        if t.strip()
    ]
    ...
    # Extract title replacement to avoid f-string nesting issues in Python 3.10
...
def export_selected_md(
    clips: List[Dict[str, Any]], folder: pathlib.Path, cfg: Dict[str, Any]
) -> int:
    ...
    for clip in clips:
        ...
        if cfg.get("md_date_prefix"):
            date = dt.datetime.fromtimestamp(clip["ts"], tz=dt.timezone.utc).strftime(
                "%Y-%m-%d"
            )
            name = f"{date}_{name}"
