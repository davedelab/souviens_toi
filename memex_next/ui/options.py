...
class OptionsWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        ttk.Checkbutton(
            ui, text="Activer les icônes flottantes", variable=float_enable
        ).pack(anchor="w", padx=12, pady=(10, 6))
        tooltips_var = tk.BooleanVar(
            value=bool(load_config().get("tooltips_enabled", True))
        )
        ttk.Checkbutton(
            ui, text="Afficher les infobulles (tooltips)", variable=tooltips_var
        ...
        ttk.Label(ui, text="Taille des icônes (px)").pack(
            anchor="w", padx=8, pady=(8, 2)
        )
        size_var = tk.IntVar(value=int(self.master.floating_icons_size))
        tk.Scale(
            ui, from_=50, to=150, resolution=5, orient="horizontal", variable=size_var
        ).pack(fill="x", padx=12)
        ...
        ttk.Checkbutton(
            ui, text="Donner le focus à  l'ouverture (icône verte)", variable=focus_var
        ).pack(anchor="w", padx=12, pady=(8, 2))
        ttk.Label(ui, text="Position par défaut (bord)").pack(
            anchor="w", padx=8, pady=(8, 2)
        )
        ...
        for lbl, val in (
            ...
            ttk.Radiobutton(side_row, text=lbl, variable=side_var, value=val).pack(
                side="left", padx=4
            )
        ...
        ttk.Button(ui, text="Repositionner (0,200)", command=reset_pos).pack(
            anchor="w", padx=12, pady=(8, 8)
        )
        sep_var = tk.BooleanVar(value=self.master.separator_enabled)
        ttk.Checkbutton(
            ui, text="Ajouter séparateur --- entre blocs (buffer)", variable=sep_var
        ...
        ttk.Entry(r1, textvariable=api_key, show="*").pack(
        ...
        ttk.Spinbox(r5, from_=1, to=12, textvariable=ai_tag_count, width=6).pack(
            side="left"
        )

        # Options PDF
        ttk.Label(
            ai, text="Analyse automatique des PDFs", font=("TkDefaultFont", 9, "bold")
        ).pack(anchor="w", padx=8, pady=(16, 4))
        auto_pdf_var = tk.BooleanVar(
            value=bool(load_config().get("auto_analyze_pdf", True))
        )
        ttk.Checkbutton(
            ai,
            text="Analyser automatiquement les PDFs avec l'IA",
            variable=auto_pdf_var,
        ).pack(anchor="w", padx=12, pady=2)
        ttk.Label(
            ai,
            text="Génère un résumé IA lors de l'import de fichiers PDF",
            font=("TkDefaultFont", 8),
        ).pack(anchor="w", padx=24, pady=(0, 8))

        # Options Web
        ttk.Label(
            ai, text="Capture web intelligente", font=("TkDefaultFont", 9, "bold")
        ).pack(anchor="w", padx=8, pady=(16, 4))
        auto_web_var = tk.BooleanVar(
            value=bool(load_config().get("auto_analyze_web", True))
        )
        ttk.Checkbutton(
            ai,
            text="Analyser automatiquement les pages web avec l'IA",
            variable=auto_web_var,
        ).pack(anchor="w", padx=12, pady=2)
        ttk.Label(
            ai,
            text="Génère un résumé IA lors de la capture d'articles web",
            font=("TkDefaultFont", 8),
        ).pack(anchor="w", padx=24, pady=(0, 4))

        save_html_var = tk.BooleanVar(
            value=bool(load_config().get("save_html_source", False))
        )
        ttk.Checkbutton(
            ai,
            text="Sauvegarder le code HTML source en pièce jointe",
            variable=save_html_var,
        ).pack(anchor="w", padx=12, pady=2)
        ttk.Label(
            ai,
            text="Conserve le HTML brut pour référence (optionnel)",
            font=("TkDefaultFont", 8),
        ).pack(anchor="w", padx=24, pady=(0, 4))
        ...
        def clean_corrupted_html():
            ...
            if corrupted:
                ...
                conn.commit()
                tk_mb.showinfo(
                    "Nettoyage", f"{len(corrupted)} fichiers HTML corrompus supprimés."
                )
            else:
        ...
        ttk.Button(
            ai,
            text="Nettoyer les fichiers HTML corrompus",
            command=clean_corrupted_html,
        ).pack(anchor="w", padx=12, pady=(4, 8))

        # ---------- Tâches ----------
        tasks_tab = ttk.Frame(nb)
        nb.add(tasks_tab, text="Tâches")
        rem_enabled_var = tk.BooleanVar(
            value=bool(load_config().get("tasks_reminders_enabled", True))
        )
        ttk.Checkbutton(
            tasks_tab, text="Activer les rappels de tâches", variable=rem_enabled_var
        ...
        rem_interval_var = tk.IntVar(
            value=int(load_config().get("tasks_reminders_interval_hours", 1))
        )
        ttk.Spinbox(row1, from_=1, to=24, textvariable=rem_interval_var, width=6).pack(
            side="left"
        )
        ...
        # Rappels par défaut
        ttk.Label(
            tasks_tab, text="Rappels par défaut", font=("TkDefaultFont", 9, "bold")
        ).pack(anchor="w", padx=8, pady=(16, 4))

        row2 = ttk.Frame(tasks_tab)
        row2.pack(fill="x", padx=8, pady=4)
        ttk.Label(row2, text="Rappel par défaut", width=24).pack(side="left")
        rem_lead_var = tk.IntVar(
            value=int(load_config().get("tasks_reminders_default_days", 1))
        )
        ttk.Spinbox(row2, from_=0, to=30, textvariable=rem_lead_var, width=6).pack(
            side="left"
        )
        ...
        ttk.Label(
            tasks_tab,
            text="Appliqué aux nouvelles tâches sans rappel personnalisé",
            font=("TkDefaultFont", 8),
        ...
        ttk.Label(cats_tab, text="Liste de catégories (0-2 par clip)").pack(
        ...
        sb = ttk.Scrollbar(
            cats_frame, orient="vertical", command=self._cats_listbox.yview
        )
        ...
        ttk.Button(actions, text="Renommer", command=rename_cat).pack(
            side="left", padx=2
        )
        ttk.Button(actions, text="Supprimer", command=delete_cat).pack(
            side="left", padx=2
        )
        ttk.Button(actions, text="Monter", command=move_up).pack(side="left", padx=2)
        ttk.Button(actions, text="Descendre", command=move_down).pack(
            side="left", padx=2
        )
        ...
        ttk.Label(
            data_tab, text="Importer depuis une ancienne base SQLite (ex: memex.db)"
        ).pack(anchor="w", padx=8, pady=(8, 4))
        pick_frame = ttk.Frame(data_tab)
        pick_frame.pack(fill="x", padx=8)
        db_path_var = tk.StringVar()
        ttk.Entry(pick_frame, textvariable=db_path_var).pack(
            side="left", fill="x", expand=True
        )
        ...
        ttk.Button(pick_frame, text="Parcourir¦", command=browse_db).pack(
            side="left", padx=6
        )
        ...
        ttk.Button(data_tab, text="Importer", command=do_import).pack(
        ...
        def apply():
            ...
            cfg["deepseek_endpoint"] = (
                endpoint.get().strip() or "https://api.deepseek.com/v1/chat/completions"
            )
            ...
            cats_vals = [
                self._cats_listbox.get(i) for i in range(self._cats_listbox.size())
            ]
            ...
            try:
                ...
                self.master._reminder_interval_ms = (
                    max(1, int(rem_interval_var.get())) * 60 * 60 * 1000
                )  # heures -> ms
                self.master._reminder_lead_sec = (
                    max(0, int(rem_lead_var.get())) * 24 * 60 * 60
                )  # jours -> secondes
            ...
            try:
                ...
                pass
            mb.showinfo(
                "Langue", "La modification de langue s'appliquera après redémarrage."
            )
            self.destroy()
