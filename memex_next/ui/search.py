...
class SearchWindow(tk.Toplevel):
    ...
    def build_tag_filters(self):
        ...
        conn = create_conn()
        rows = conn.execute(
            "SELECT tags FROM clips WHERE tags IS NOT NULL AND tags <> ''"
        ).fetchall()
        conn.close()
        all_tags = set()
        for (t,) in rows:
        ...
        conn = create_conn()
        for tag in all_tags:
            counts[tag] = conn.execute(
                "SELECT COUNT(*) FROM clips WHERE tags LIKE ?", (f"%{tag}%",)
            ).fetchone()[0]
        conn.close()
        for tag, _ in sorted(counts.items(), key=lambda x: x[1], reverse=True)[:20]:
            btn = tk.Button(
                self.tags_filter_frame,
                text=f"{tag} ({counts[tag]})",
                relief="raised",
                bd=1,
                padx=4,
                pady=2,
    ...
    def build_category_filters(self):
        ...
        for cat in user_cats:
            counts[cat] = conn.execute(
                "SELECT COUNT(*) FROM clips WHERE categories LIKE ?", (f"%{cat}%",)
            ).fetchone()[0]
        conn.close()
        for cat in user_cats:
            btn = tk.Button(
                self.cats_filter_frame,
                text=f"{cat} ({counts.get(cat, 0)})",
                relief="raised",
                bd=1,
                padx=4,
                pady=2,
