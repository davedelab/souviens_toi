...
from ..config import SEPARATOR, load_config
...
class EditClipWindow(tk.Toplevel):
    ...
    def _create_widgets(self):
        ...
        ttk.Button(row1, text="AI", width=4, command=self._ai_title).pack(
            side="left", padx=4
        )
        ...
        ttk.Button(row2, text="AI", width=4, command=self._ai_tags).pack(
            side="left", padx=4
        )
        ...
        self.cat1_cb = ttk.Combobox(
            row3, values=self._user_cats, textvariable=self.cat1_var, state="readonly"
        )
        self.cat2_cb = ttk.Combobox(
            row3, values=self._user_cats, textvariable=self.cat2_var, state="readonly"
        )
        self.cat1_cb.pack(side="left", fill="x", expand=True)
        self.cat2_cb.pack(side="left", fill="x", expand=True, padx=(6, 0))
        ttk.Button(row3, text="AI", width=4, command=self._ai_categories).pack(
            side="left", padx=4
        )
        ...
        ttk.Checkbutton(
            row4, text="A lire plus tard", variable=self.read_later_var
        ).pack(anchor="w")
        ...
        btn(
            "%",
            "Préserver (pour résumé IA)",
            lambda: self._md_preserve(),
            w=3,
            bg="#f97316",
        )
        ...
        self.editor = st.ScrolledText(
            left, wrap="word", undo=True, autoseparators=True, maxundo=1000
        )
        ...
        self._thumb_canvas = tk.Canvas(
            self._tab_images, width=260, height=520, highlightthickness=0
        )
        self._thumb_scroll = ttk.Scrollbar(
            self._tab_images, orient="vertical", command=self._thumb_canvas.yview
        )
        ...
        self._thumb_canvas.create_window(
            (0, 0), window=self._thumb_container, anchor="nw"
        )
        ...
        ttk.Button(af, text="Ouvrir", command=self._open_attachment_selected).pack(
            side="left", expand=True, fill="x"
        )
        ttk.Button(af, text="Exporter", command=self._export_attachment_selected).pack(
            side="left", expand=True, fill="x", padx=2
        )
        ttk.Button(af, text="Supprimer", command=self._delete_attachment_selected).pack(
            side="left", expand=True, fill="x"
        )
        ttk.Button(
            self._tab_attach,
            text="Joindre fichier",
            command=self._attach_files_to_current_clip,
        ...
        ttk.Button(btn_frame, text="Prévisualiser MD", command=self._preview_md).pack(
            side="left", padx=(8, 2)
        )
        ttk.Button(
            btn_frame, text="Aperçu intégré", command=self._preview_md_embedded
        ).pack(side="left")
        ttk.Button(btn_frame, text="Résumé IA", command=self._ai_smart_summary).pack(
            side="left", padx=(8, 0)
        )
    ...
    def _bind_shortcuts(self):
        for keys, func in (
            ...
            ("<Control-b>", lambda e: (self._md_bold(), "break")),
            ("<Control-i>", lambda e: (self._md_italic(), "break")),
            ("<Control-k>", lambda e: (self._md_link(), "break")),
            ("<Control-z>", lambda e: (self._undo_editor(), "break")),
            ("<Control-y>", lambda e: (self._redo_editor(), "break")),
            ("<Control-1>", lambda e: (self._md_h1(), "break")),
            ("<Control-2>", lambda e: (self._md_h2(), "break")),
            ("<Control-3>", lambda e: (self._md_h3(), "break")),
        ):
            self.bind(keys, func)
    ...
    def _apply_prefill(self, prefill: Dict[str, Any]):
        ...
        parts = [
            p.strip() for p in (prefill.get("categories") or "").split(",") if p.strip()
        ]
    ...
    def _load(self):
        conn = create_conn()
        row = conn.execute(
            "SELECT title, raw_text, tags, categories, read_later FROM clips WHERE id=?",
            (self.clip_id,),
        ).fetchone()
        conn.close()
        if not row:
    ...
    def _save(self):
        ...
        cats = ", ".join(
            [p for p in (self.cat1_var.get(), self.cat2_var.get()) if p.strip()]
        )
    ...
    def _preview_md(self):
        ...
        html_body = (
            _markdown.markdown(text, extensions=["extra", "sane_lists", "nl2br"])
            if _markdown
            else f"<pre>{text}</pre>"
        )
        html_doc = (
    ...
    def _preview_md_embedded(self):
        ...
        if _HTMLLabel:
            html_container = _HTMLLabel(
                win,
                html=f"<div style='font-family:Segoe UI,Arial,sans-serif;line-height:1.6'>{html_body}</div>",
            )
    ...
    def _ai_tags(self):
        ...
        def done(res, err):
            ...
            if (
                "Non traitée par l'IA" in current_tags
                or "non traitée par l'IA" in current_tags
            ):
                existing = []
            else:
                existing = [
                    p.strip()
                    for p in current_tags.replace(";", ",").split(",")
                    if p.strip()
                ]
    ...
    def _ai_categories(self):
        ...
        def work():
            return ai_generate_categories(
                text,
                user_cats=self._user_cats,
                lang=self.cfg.get("ai_lang", "fr"),
                max_n=2,
            )
    ...
    def _ai_all(self):
        ...
        def work():
            ...
            cats = (
                ai_generate_categories(text, user_cats=user_cats, lang=lang, max_n=2)
                if user_cats
                else []
            )
        ...
        def done(res, err):
            ...
            if res.get("tags") is not None:
                existing = [
                    p.strip()
                    for p in (self.tags_var.get() or "").replace(";", ",").split(",")
                    if p.strip()
                ]
    ...
    def _attach_file_classic_editor(self, mime, data):
        ...
        def work(mime=mime, blob=data):
            ...
            if text:
                conn = create_conn()
                row = conn.execute(
                    "SELECT raw_text FROM clips WHERE id=?", (self.clip_id,)
                ).fetchone()
    ...
    def _reload_thumbnails(self):
        ...
        conn = create_conn()
        rows = conn.execute(
            "SELECT id, filename, mime, data FROM files WHERE clip_id=? ORDER BY id DESC",
            (self.clip_id,),
        ).fetchall()
        conn.close()
        for fid, fn, mime, blob in rows:
            if (
                ...
                try:
                    ...
                    lbl.bind(
                        "<Button-1>",
                        lambda e, b=blob, t=fn: self._open_image_preview(b, t),
                    )
                ...
                ttk.Label(card, text=fn or f"PDF #{fid}").pack(
                    side="top", anchor="w", padx=4, pady=(4, 0)
                )
                btns = ttk.Frame(card)
                ...
                ttk.Button(
                    btns,
                    text="Ouvrir",
                    command=lambda i=fid: self._open_attachment_by_id(i),
                ).pack(side="left")
                ttk.Button(
                    btns,
                    text="Exporter",
                    command=lambda i=fid: self._export_attachment_by_id(i),
                ).pack(side="left", padx=4)
                ttk.Button(
                    btns,
                    text="Supprimer",
                    command=lambda i=fid: self._delete_attachment_by_id(i),
                ).pack(side="left")
                self._has_pdfs = True
    ...
    def _load_attachments_list(self):
        ...
        conn = create_conn()
        rows = conn.execute(
            "SELECT id, filename, size, mime FROM files WHERE clip_id=? ORDER BY id DESC",
            (self.clip_id,),
        ).fetchall()
        conn.close()
    ...
    def _open_attachment_by_id(self, fid):
        conn = create_conn()
        row = conn.execute(
            "SELECT filename, data, mime FROM files WHERE id=?", (fid,)
        ).fetchone()
        conn.close()
        if not row:
            return
    ...
    def _export_attachment_by_id(self, fid):
        conn = create_conn()
        row = conn.execute(
            "SELECT filename, data FROM files WHERE id=?", (fid,)
        ).fetchone()
        conn.close()
        if not row:
            return
        ...
        path = fd.asksaveasfilename(
            initialfile=fn, defaultextension=pathlib.Path(fn).suffix or ".pdf"
        )
        if not path:
            return
