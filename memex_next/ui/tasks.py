...
class TasksWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        ttk.Entry(top, textvariable=self.new_title).pack(
        ...
        self.reminder_days = tk.Spinbox(
            reminder_wrap, from_=0, to=30, width=4, format="%d"
        )
        ...
        ttk.Button(btns, text="Supprimer", command=self._delete).pack(
            side="left", padx=6
        )
        ttk.Button(btns, text="Rafraîchir", command=self._refresh).pack(side="left")
        ttk.Button(
            btns, text="Modifier rappel", command=self._set_reminder_selected
        ).pack(side="right", padx=6)
        ttk.Button(btns, text="Définir échéance", command=self._set_due_selected).pack(
            side="right"
        )
    ...
    def _refresh(self):
        ...
        conn = create_conn()
        rows = conn.execute(
            "SELECT id, title, status, priority, due_at, reminder_days FROM tasks ORDER BY COALESCE(due_at, 1e18) ASC, id DESC"
        ).fetchall()
        conn.close()
        for rid, title, status, prio, due, reminder_days in rows:
            ...
            if due:
                try:
                    due_s = dt.datetime.fromtimestamp(due, tz=dt.timezone.utc).strftime(
                        "%Y-%m-%d %H:%M"
                    )
                except Exception:
                    pass
            ...
            self.tree.insert(
                ...
                "end",
                iid=str(rid),
                values=(rid, title, status, prio, due_s, reminder_s),
            )
    ...
    def _add(self):
        ...
        try:
            reminder_days = (
                int(self.reminder_days.get())
                if self.reminder_days.get().strip()
                else None
            )
            if reminder_days == 0:
        ...
        if due_ts is not None:
            conn.execute(
                "INSERT INTO tasks(title, status, priority, due_at, reminder_days, created_at) VALUES(?,?,?,?,?,?)",
                (
                    title,
                    "pending",
                    "medium",
            ...
            conn.execute(
                "INSERT INTO tasks(title, status, priority, reminder_days, created_at) VALUES(?,?,?,?,?)",
                (
                    title,
                    "pending",
                    "medium",
    ...
    def _set_reminder_selected(self):
        ...
        conn = create_conn()
        row = conn.execute(
            "SELECT title, reminder_days FROM tasks WHERE id=?", (tid,)
        ).fetchone()
        conn.close()
        ...
        ttk.Label(reminder_frame, text="jour(s) avant l'échéance").pack(
            side="left", padx=(4, 0)
        )

        ttk.Label(frm, text="(0 = aucun rappel)", font=("TkDefaultFont", 8)).pack(
            anchor="w", pady=(4, 8)
        )
        ...
        def apply_reminder():
            try:
                ...
                conn = create_conn()
                conn.execute(
                    "UPDATE tasks SET reminder_days=? WHERE id=?", (new_reminder, tid)
                )
                conn.commit()
                conn.close()
        ...
        ttk.Button(btns_frame, text="Appliquer", command=apply_reminder).pack(
            side="right"
        )
        ttk.Button(btns_frame, text="Annuler", command=dlg.destroy).pack(
            side="right", padx=(0, 6)
        )
    ...
    def _edit_due_inline(self, event):
        ...
        ttk.Button(btns, text="Annuler", command=dlg.destroy).pack(
            side="right", padx=(0, 6)
        )
        dlg.grab_set()
        dlg.focus_force()
