...
class BufferApp(tk.Tk):
    ...
    def attach_file(self):
        ...
        import mimetypes
        added = 0
        self._first_clip_id_for_session_for_session = None
                ...
                if is_pdf and auto_analyze_pdf:
                    ...
                    def done_pdf(pdf_result, err):
                        ...
                        if pdf_result and pdf_result.get('success'):
                            ...
                            clip_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
                            if self._first_clip_id_for_session_for_session is None:
                                self._first_clip_id_for_session_for_session = clip_id
        ...
        if added:
            ...
            if self._first_clip_id_for_session_for_session:
                self.after(100, lambda: EditClipWindow(self, self._first_clip_id_for_session_for_session))
    ...
    def _attach_file_classic(self, file_path, data, sha, mime, title):
        ...
        clip_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        if not hasattr(self, '_first_clip_id') or self._first_clip_id_for_session is None:
            self._self._first_clip_id_for_session_for_session = clip_id
        conn.execute("INSERT OR IGNORE INTO files(clip_id, filename, mime, size, sha256, data) VALUES (?,?,?,?,?,?)",
                     (clip_id, title, mime, len(data), sha, data))
        conn.commit()
        conn.close()
