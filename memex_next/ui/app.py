...
class BufferApp(tk.Tk):
    ...
    def build_ui(self):
        ...
        btn_md = tbtn(
            top, _tr("md"), self.capture_selection_markdown, w=6, bg="#0ea5e9"
        )
        ...
        btn(
            tb, "Redo", "Rétablir (Ctrl+Y)", lambda: self._redo_buf(), w=5, bg="#374151"
        )
    ...
    def attach_file(self):
                ...
                if is_pdf and auto_analyze_pdf:
                    ...
                    def done_pdf(pdf_result, err):
                        ...
                        if pdf_result and pdf_result.get("success"):
                            ...
                            if (
                                not hasattr(self, "_first_clip_id_for_session")
                                or self._first_clip_id_for_session is None
                            ):
        ...
        if added:
            ...
            if (
                hasattr(self, "_first_clip_id_for_session")
                and self._first_clip_id_for_session
            ):
                fid = self._first_clip_id_for_session
                self._first_clip_id_for_session = None
                self.after(100, lambda: EditClipWindow(self, fid))
