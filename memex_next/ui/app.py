...
_LANG = {
    "fr": {
        ...
        "float_show": "Voir",
        "float_hide": "Cacher",
        "float_search": "Recherche",
        "tt_add": "Ajouter le presse-papiers au buffer",
        "tt_save": "Enregistrer le buffer dans la base",
        "tt_file": "Joindre un fichier (PDF, image, etc.) au nouveau clip",
        "tt_article": "Capturer un article Web (extraction + Markdown)",
        "tt_md": "Coller la sélection Web au format Markdown",
        "tt_search": "Ouvrir la recherche",
        "tt_pin": "Epingler la fenêtre (toujours au-dessus)",
        "tt_tasks": "Ouvrir les tâches",
        "tt_options": "Ouvrir les options",
        "undo": "Annuler",
        "redo": "Rétablir",
    },
    "en": {
        ...
        "float_hide": "Hide",
        "float_search": "Search",
        "tt_add": "Add clipboard to buffer",
        "tt_save": "Save buffer to database",
        "tt_file": "Attach a file (PDF, image, etc.) to the new clip",
        "tt_article": "Capture a web article (extraction + Markdown)",
        "tt_md": "Paste web selection as Markdown",
        "tt_search": "Open search",
        "tt_pin": "Pin window (always on top)",
        "tt_tasks": "Open tasks",
        "tt_options": "Open options",
        "undo": "Undo",
        "redo": "Redo",
    },
}
...
class BufferApp(tk.Tk):
    ...
    def capture_selection_markdown(self):
        ...
        if not txt:
            ...
            messagebox.showinfo(
                "Markdown",
                "Presse-papiers vide. Copiez d'abord une sélection depuis votre navigateur.",
            )
            return
        looks_html = (
            "<" in txt and ">" in txt and ("</" in txt or "<p" in txt or "<div" in txt)
        )
        if looks_html:
            try:
        ...
        if (
            ...
            and current
            and not current.endswith(SEPARATOR.strip())
        ):
    ...
    def __init__(self):
        ...
        self.bind_all(
            "<Control-Alt-t>", lambda e: self.set_title_from_selection_or_clipboard()
        )
    ...
    def build_ui(self):
        ...
        self.pause_btn = ttk.Button(
            top, text=_tr("pause_short"), width=4, command=self.toggle_pause
        )
        self.pause_btn.pack(side="left", padx=2)

        self._make_ui_button(
            top,
            _tr("add"),
            self.add_clipboard,
            tip=_tr("tt_add"),
        ...
        self._make_ui_button(
            top, _tr("save"), self.send_all, tip=_tr("tt_save"), width=10, bg="#16a34a"
        )
        self._make_ui_button(
            top,
            _tr("article"),
            self.capture_article,
            tip=_tr("tt_article"),
            width=8,
            bg="#2563eb",
        )
        self._make_ui_button(
            top,
            _tr("md"),
            self.capture_selection_markdown,
            tip=_tr("tt_md"),
            width=6,
            bg="#0ea5e9",
        )
        ...
        self.pin_btn = self._make_ui_button(
            top,
            _tr("pin_on"),
        ...
        ttk.Entry(frm_title, textvariable=self.title_var).pack(
        ...
        ttk.Button(
            ...
            text="Titre",
            width=5,
            command=self.set_title_from_selection_or_clipboard,
        ).pack(side="right", padx=2)
        ttk.Button(
            frm_title, text="AI", width=3, command=self.ai_title_from_buffer
        ).pack(side="right", padx=2)
        self.read_later_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            frm_title, text="A lire plus tard", variable=self.read_later_var
        ...
        ttk.Button(
            tags_row, text="AI", width=3, command=self.ai_fill_tags_from_buffer
        ...
        self.cat1_cb_buf = ttk.Combobox(
            cats_row,
            values=self._user_cats_main,
            textvariable=self.cat1_var_buf,
            state=state_main,
            width=24,
        )
        self.cat2_cb_buf = ttk.Combobox(
            cats_row,
            values=self._user_cats_main,
            textvariable=self.cat2_var_buf,
            state=state_main,
            width=24,
        )
        ...
        ttk.Button(
            cats_row, text="AI", width=3, command=self.ai_fill_categories_from_buffer
        ...
        self._make_ui_button(
            tb,
            "`",
            lambda: self._md_code_inline_buf(),
            tip="Code inline",
            width=3,
            bg="#6b7280",
        )
        self._make_ui_button(
            tb,
            "```",
            lambda: self._md_code_block_buf(),
            tip="Bloc de code",
            width=5,
            bg="#6b7280",
        )
        self._make_ui_button(
            tb,
            "H1",
            lambda: self._md_h1_buf(),
            tip="Titre niveau 1 (Ctrl+1)",
            width=4,
            bg="#fbbf24",
        )
        self._make_ui_button(
            tb,
            "H2",
            lambda: self._md_h2_buf(),
            tip="Titre niveau 2 (Ctrl+2)",
            width=4,
            bg="#f59e0b",
        )
        self._make_ui_button(
            tb,
            "H3",
            lambda: self._md_h3_buf(),
            tip="Titre niveau 3 (Ctrl+3)",
            width=4,
            bg="#d97706",
        )
        self._make_ui_button(
            tb,
            "*",
            lambda: self._md_bullet_buf(),
            tip="Liste à  puces",
            width=3,
            bg="#8b5cf6",
        )
        self._make_ui_button(
            tb, ">", lambda: self._md_quote_buf(), tip="Citation", width=3, bg="#ef4444"
        )
        self._make_ui_button(
            tb,
            "HR",
            lambda: self._md_hr_buf(),
            tip="Ligne horizontale",
            width=4,
            bg="#10b981",
        )
        self._make_ui_button(
        ...
        self.text_area = scrolledtext.ScrolledText(
            self, wrap="word", undo=True, autoseparators=True, maxundo=1000
        )
    ...
    def _update_pause_ui(self):
        ...
        self.pause_btn.config(
            text=_tr("run_short") if self.paused else _tr("pause_short")
        )
    ...
    def send_all(self):
        ...
        cats = ", ".join(
            {self.cat1_var_buf.get().strip(), self.cat2_var_buf.get().strip()} - {""}
        )
        ...
        if source:
            conn.execute(
                "INSERT OR IGNORE INTO source_urls(url, clip_id, created_at) VALUES (?,?,?)",
    ...
    def _md_wrap_buf(self, left, right, placeholder=""):
        ...
        if start and end:
            ...
            self.text_area.tag_add(
                "sel", start, f"{start}+{len(left) + len(text) + len(right)}c"
            )
        else:
    ...
    def _md_link_buf(self):
        ...
        label = (
            txt
            or simpledialog.askstring("Lien", "Texte du lien:", initialvalue=url)
            or url
        )
        if start and end:
            self.text_area.delete(start, end)
    ...
    def ai_fill_categories_from_buffer(self):
        ...
        def work():
            ...
            picked = (
                ai_generate_categories(
                    text, user_cats=user_cats, lang=cfg.get("ai_lang", "fr"), max_n=1
                )
                or []
            )
            sugg = (
                ai_suggest_new_categories(
                    text,
                    existing_list=user_cats,
                    lang=cfg.get("ai_lang", "fr"),
                    max_n=1,
                )
                or []
        ...
        def done(res, err):
            ...
            self.cat1_var_buf.set(
                from_list[0] if len(from_list) > 0 else self.cat1_var_buf.get()
            )
            self.cat2_var_buf.set(
                suggested[0] if len(suggested) > 0 else self.cat2_var_buf.get()
            )
    ...
    def capture_article(self):
        ...
        try:
            default_url = self.last_source_url or (
                ...
                if self._looks_like_url(pyperclip.paste().strip())
                else ""
            )
        except Exception:
            default_url = self.last_source_url or ""
        url = simpledialog.askstring(
            "Capture Web IA",
            "URL de la page à capturer et analyser:",
    ...
    def _generate_web_tags_async(self, clip_id: int, content: str):
        ...
        def work():
            ...
            current_row = conn.execute(
                "SELECT tags, categories FROM clips WHERE id=?", (clip_id,)
            ).fetchone()
            if current_row:
                ...
                existing_tags = [
                    t.strip()
                    for t in current_tags.replace(";", ",").split(",")
                    if t.strip()
                ]
                existing_cats = [
                    c.strip() for c in current_cats.split(",") if c.strip()
                ]
                ...
                conn.execute(
                    "UPDATE clips SET tags=?, categories=? WHERE id=?",
        ...
        for p in paths:
            try:
                ...
                if is_pdf and auto_analyze_pdf:
                    ...
                    self.show_toast(
                        "📄 Numérisation PDF IA en cours... Veuillez patienter"
                    )
                    ...
                    def done_pdf(pdf_result, err):
                        if err:
                            ...
                            self._attach_file_classic(
                                {"data": data, "sha": sha, "mime": mime, "title": title}
                            )
                            return
                    ...
                    runner.submit(
                        work_pdf, cb=lambda r, e: self.after(0, done_pdf, r, e)
                    )
                else:
                ...
                messagebox.showerror(
                    "Import", f"Echec import {pathlib.Path(p).name}: {e}"
                )
        if added:
    ...
    def _create_floating_icons(self):
        ...
        make_float_btn(
            ...
            _tr("float_hide"),
            _tr("float_hide"),
            self._float_red_click,
            bg="#ef4444",
        )
        make_float_btn(
            frm, "🔍", _tr("float_search"), self._float_search_click, bg="#3b82f6"
        )
    ...
    def _reminder_setup(self):
        ...
        self._reminder_interval_ms = (
            max(1, interval_hours) * 60 * 60 * 1000
        )  # heures -> ms
