...
def ai_generate_tags(text: str, lang: str = "fr", count: int = 5) -> list[str]:
    ...
    sys = {
        "role": "system",
        "content": f'Tu extrais {count} tags concis en {lang}. Réponds JSON: {{"tags":[]}}',
    }
    ...
    try:
        ...
        if "tags" in parsed and isinstance(parsed["tags"], list):
            return [
                t.strip() for t in parsed["tags"] if isinstance(t, str) and t.strip()
            ][:count]
    except Exception:
        pass
...
def ai_generate_title(text: str, lang: str = "fr", max_len: int = 80) -> str:
    ...
    sys = {
        "role": "system",
        "content": f"Tu es un assistant qui propose des titres concis en {lang}.",
    }
    user = {
        "role": "user",
        "content": f"Génère un titre court (={max_len} car.) sans guillemets :\\n\\n{text}",
    }
    out = _ai_call([sys, user], MODEL, key, cfg.get("deepseek_endpoint", ENDPOINT))
...
def ai_generate_categories(
    text: str, user_cats: list[str], lang: str = "fr", max_n: int = 2
) -> list[str]:
    cfg = load_config()
    key = cfg.get("deepseek_api_key")
    ...
    sys = {
        "role": "system",
        "content": f'Tu choisis 0 à {max_n} catégorie(s) parmi la liste fournie en {lang}. Réponds JSON: {{"categories":[]}}',
    }
    cats_join = ", ".join(user_cats)
    user = {
        "role": "user",
        "content": f"Liste: [{cats_join}]\\n\\nTexte:\\n{text}\\n\\nJSON:",
    }
    out = _ai_call([sys, user], MODEL, key, cfg.get("deepseek_endpoint", ENDPOINT))
    try:
        chosen = [
            c.strip() for c in json.loads(out)["categories"] if c.strip() in user_cats
        ]
        return chosen[:max_n]
    except Exception:
        return []
...
def ai_smart_summary(text: str, lang: str = "fr") -> str:
    ...
    for i, match in enumerate(reversed(matches)):  # Inverser pour préserver les indices
        ...
        text_to_summarize = (
            text_to_summarize[: match.start()]
            + marker
            + text_to_summarize[match.end() :]
        )
...
def ai_suggest_new_categories(
    text: str, existing_list: list[str], lang: str = "fr", max_n: int = 3
) -> list[str]:
    ...
    sys_content = (
        f"Tu suggères {max_n} nouvelles catégories en {lang} pour classer ce texte. "
    )
        ...
        if "categories" in parsed and isinstance(parsed["categories"], list):
            ...
            for cat in parsed["categories"]:
                if isinstance(cat, str) and cat.strip():
                    cat_clean = cat.strip()
                    if cat_clean.lower() not in [
                        existing.lower() for existing in existing_list
                    ]:
